...
import $ from "jquery";
import { launchLoginUrl } from "../managers/UserStatusManager";
...
export function showTextInputPrompt(title, textContent, listenerId, defaultInput) {
  ...
  inputPrompt.classList.add("inputPopup");
...
export function showListSelector(items, title, textContent, listenerId) {
  ...
  listSelector.classList.add("listPopup");
...
$(document).on('change', '#authSelectId', function() {
  ...
  if (data) {
    ...
    launchLoginUrl(data.value, switching_account);
  }
  closeListSelector();
});
