...
const TimeCounterStats = require("../models/TimeCounterStats");
const KeystrokeAggregate = require("../models/KeystrokeAggregate");
...
const fileChangeInfoSummaryDataMgr = require("./FileChangeInfoSummaryDataManager");
...
pluginDataMgr.processPayloadHandler = async (payload, sendNow, nowTimes, isUnfocusEvent = false) => {
  ...
  fileUtil.setItem("latestPayloadTimestampEndUtc", latestPayloadTimestampEndUtc);
...
function updateFileData() {
  if (stats) {
    // store the file data
    fileUtil.storeJsonData(utilMgr.getTimeCounterFile(), stats)
  }
}

function fetchTimeCounterStats() {
  const timeCounterJson = fileUtil.getJsonData(utilMgr.getTimeCounterFile());
  if (timeCounterJson) {
    stats = {
      ...timeCounterJson
    };
  ...
  if (keys && keys.length) {
    for (let i = 0; i < keys.length; i++) {
      ...
      if (payload.project && payload.project.identifier) {
        ...
        const repoFileContributorCount = await repoMgr.getFileContributorCount(key);
...
async function updateCumulativeSessionTime(payload, sessionSeconds) {
  ...
  let lastPayload = await fileUtil.getLastSavedKeystrokesStats(utilMgr.getSoftwareDataStoreFile());
  ...
  payload.workspace_name = utilMgr.getEditorSessionToken();
...
async function midnightCheckHandler() {
  if (utilMgr.isNewDay()) {
    ...
    fileUtil.setItem('currentDay', nowTime.day);
...
async function populateRepoMetrics(payload) {
  if (payload.project && payload.project.identifier && payload.project.directory) {
    ...
    const repoContributorInfo = await repoMgr.getRepoContributorInfo(
    ...
    // REPO file count
    const repoFileCount = await repoMgr.getRepoFileCount(payload.project.directory);
