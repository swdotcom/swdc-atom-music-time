...
import { requiresSpotifyAccess } from "./SpotifyManager";
import {
    app_endpoint,
    api_endpoint,
...
utilMgr.updateLoginPreference = spotifyConnected => {
    ...
    if (spotifyConnected) {
        ...
        structureViewObj.toggleConnectSpotify();
...
utilMgr.populateLikedSongs = async (refresh = true) => {
    if (MusicDataManager.getInstance().spotifyLikedSongs.length === 0 || refresh) {
