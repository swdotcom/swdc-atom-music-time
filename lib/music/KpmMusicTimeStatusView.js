...
export default class KpmMusicTimeStatusView {
    ...
    async display() {
        ...
        if (track && track.id && !requiresSpotifyAccess && this.dataMgr.isSpotifyPremium) {
            hasTrackInfo = true;
        }
        ...
        this.playElement.innerHTML =
            "<span id='music-play-control-href' class='play-control-panel' style='cursor: pointer;' title='" + playTooltip + "'><img width='10' height='10' id='play-image' src='" + songStateIcon + "'/></span>";

        this.nextElement.innerHTML =
            "<span id='music-next-control-href' class='play-control-panel' style='cursor: pointer;' title='" + nextTooltip + "'><img width='10' height='10' src='" + NEXT_CONTROL_ICON + "'/></span>";

        this.prevElement.innerHTML =
            "<span id='music-prev-control-href' class='play-control-panel' style='cursor: pointer;' title='" + previousTooltip + "'><img width='10' height='10' src='" + PREV_CONTROL_ICON + "'/></span>";

        if (currentTrackName) {
            currentTrackName = stringUtil.text_truncate(currentTrackName, 15)
            this.currentTrack.innerHTML =
                "<span id='music-current-control-href' class='play-control-panel' style='cursor: pointer;' title='" + currentTrackTooltip + "'>" +
                currentTrackName +
                '</span>';
        }
