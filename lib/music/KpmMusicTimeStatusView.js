...
export default class KpmMusicTimeStatusView {
    ...
    async display(msg, icon, tooltip, track) {
        ...
        let connectMsg = "";
        if (fileUtil.requiresSpotifyReAuthentication()) {
           connectMsg = "Reconnect Spotify";
        } else if (fileUtil.requiresSpotifyAccess()) {
            connectMsg = "Connect Spotify";
        }

        let headphonesHtml = "<span id='music-time-menu' style='cursor: pointer;' title='" + headphonesTooltip + "'>" + msg + "</span>";

        if (connectMsg) {
            headphonesHtml += "<span id='music-time-connect' style='cursor:pointer;padding-left:4px;'>" + connectMsg + "</span>";
        }
        ...
        if (
            ...
            track.actions.disallows.skipping_prev
        ) {
            skipping_prev = true;
        }
        if (!skipping_prev) {
        ...
        const needsSpotifyAccess = fileUtil.requiresSpotifyAccess();
