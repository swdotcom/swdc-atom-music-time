...
export default class KpmMusicTimeStatusView {
    ...
    clearStatus() {
        ...
        let likeNode = document.getElementById('likeSong');
        let trackNode = document.getElementById('current-track-status');
        let statusNode = document.getElementById('music-time-status');
        ...
        if (footerBars && footerBars.length) {
            ...
            if (likeNode) {
                footerBars[0].getItem().leftPanel.removeChild(likeNode);
            }
            if (trackNode) {
                footerBars[0].getItem().leftPanel.removeChild(trackNode);
            }
            if (statusNode) {
                footerBars[0].getItem().leftPanel.removeChild(statusNode);
            }
        }
    }
    ...
    async display(msg, icon, tooltip, track) {
        ...
        let headphonesHtml = "<span id='music-time-menu' style='cursor: pointer;' title='" + headphonesTooltip + "'><img width='10' height='10' src='" + HEADPHONES + "'/></span>";
        ...
        if (footerBars && footerBars.length > 0) {
            ...
            if (connectMsg) {
                this.statusSpan.innerHTML = "<span id='music-time-connect' style='cursor:pointer;padding-left:4px;'>" + connectMsg + "</span>";
        ...
        if (this.trackData) {
            currentTrackTooltip = `(${this.trackData.name}) Click to view track`;
        }
        ...
        this.playElement.innerHTML =
            "<span id='music-play-control-href' style='cursor: pointer;' title='" + playTooltip + "'><img width='10' height='10' id='play-image' src='" + songStateIcon + "'/></span>";

        this.nextElement.innerHTML =
            "<span id='music-next-control-href' style='cursor: pointer;' title='" + nextTooltip + "'><img width='10' height='10' src='" + NEXT_CONTROL_ICON + "'/></span>";

        this.prevElement.innerHTML =
            "<span id='music-prev-control-href' style='cursor: pointer;' title='" + previousTooltip + "'><img width='10' height='10' src='" + PREV_CONTROL_ICON + "'/></span>";

        // there's nothing to get if it's windows and they don't have
        // a premium spotify account
        const isMac = fileUtil.isMac();

        let premiumAccountRequired = this.dataMgr
            ? isMac && !this.dataMgr.hasSpotifyPlaybackAccess
                ? true
                : false
            : true;
        ...
        if (currentTrackName) {
            this.currentTrack.innerHTML =
                ...
                iconClass + "' style='cursor: pointer;' title='" + currentTrackTooltip + "'>" +
                currentTrackName +
                '</span>';
        }
