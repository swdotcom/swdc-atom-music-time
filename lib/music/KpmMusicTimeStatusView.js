...
export default class KpmMusicTimeStatusView {
    ...
    async display(msg, icon, tooltip, track) {
        ...
        this.musicIcon.innerHTML =
            "<span id='music-time-menu' style= 'cursor: pointer;'>" + msg +
            "</span>";

        if (footerBars && footerBars.length > 0 && !track) {
        ...
        let songStateIcon =
            track  && track.state == 'playing' ? PAUSE_CONTROL_ICON : PLAY_CONTROL_ICON;

        this.playElement.innerHTML =
            "<span id='music-play-control-href' class=' play-control-panel" +
            iconClass +
            '\' style="cursor: pointer;" title=\'' +
            playTooltip +
            "'><img width='10' height='10' id='play-image' src='" +
            songStateIcon +
            "'/></span>";

        this.nextElement.innerHTML =
            "<span id='music-next-control-href' class=' play-control-panel" +
            iconClass +
            '\' style="cursor: pointer;" title=\'' +
            nextTooltip +
            ...
            NEXT_CONTROL_ICON +
            "'/></span>";
        let skipping_prev = false;
        if(track && track.actions && track.actions.disallows && track.actions.disallows.skipping_prev) {
            skipping_prev = true;
        }
        if(!skipping_prev) {
            this.prevElement.innerHTML =
                "<span id='music-prev-control-href' class='play-control-panel" +
                iconClass +
                '\' style="cursor: pointer;" title=\'' +
                previousTooltip +
                "'><img width='10' height='10' src='" +
            ...
            this.prevElement.innerHTML =
                "<span id='music-prev-control-href' class='play-control-panel' style=' opacity: 0.4;background-color: rgb(35, 34, 34);' ><img width='10' height='10' src='" +PREV_CONTROL_ICON +"'/></span>";
            this.prevElement.setAttribute('onclick',false);
            this.prevElement.setAttribute('title','Disabled');
        }

        // there's nothing to get if it's windows and they don't have
        // a premium spotify account
        let premiumAccountRequired = this.dataMgr
            ? utilMgr.isMac() && !this.dataMgr.hasSpotifyPlaybackAccess
                ? true
                : false
            : true;
        ...
        if (currentTrackName) {
            this.currentTrack.innerHTML =
                "<span id='music-current-control-href' class=' play-control-panel" +
                iconClass +
                '\' style="cursor: pointer;" title=\'' +
                tooltip +
                "'>" +
        ...
        if (utilMgr.getItem('isSpotifyConnected') && premiumAccountRequired) {
        ...
        if (foundLiked && track && track.state !== 'notassigned') {
            this.likeStatus.innerHTML =
                "<span id='unlike-status-href' class='play-control-panel' style='cursor: pointer;' title='" +
                likeStatusMsg +
            ...
            this.likeStatus.innerHTML =
                "<span id='like-status-href'  class='play-control-panel' style='cursor: pointer;' title='" +
                likeStatusMsg +
                "'><img width='13' height='13' src='" +
                LIKE_ICON_OUTLINE +
                "'/></span>";
        ...
        let needsSpotifyAccess = this.dataMgr
            ? this.dataMgr.requiresSpotifyAccess
            : true;

        this.element.innerHTML =
            "<span id='music-time-connect'  style='cursor: pointer;' title='" +
            tooltip +
            "' >  Connect Spotify  </span>";
        if (!needsSpotifyAccess) {
            this.element.setAttribute('style', 'display:none;');
        } else {
            this.element.setAttribute('style', 'display:block;');
        }
        if (track && track.state !== 'notassigned' && !needsSpotifyAccess) {
        ...
        if (utilMgr.getItem('isSpotifyConnected') && premiumAccountRequired) {
            footerBars[0].getItem().leftPanel.appendChild(this.accountStatus);
            utilMgr.addMusicMenuItem(
                MUSIC_CONNECT_PREMIUM,
                CONNECT_SPOTIFY_COMMAND_KEY
            );
        } else {
            utilMgr.removeMusicMenuItem(MUSIC_CONNECT_PREMIUM);
        }
        // if (track.state == 'paused') {
        //     $('#play-image').attr('src', PLAY_CONTROL_ICON);
        // } else if (track.state == 'playing') {
