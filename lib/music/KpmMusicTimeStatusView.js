...
$(document).ready(function() {
    ...
    $(document).on('click', '#unlikeSong', async function() {
        // {loggedIn: true|false}
        if (!userStatus || !userStatus.loggedIn) {
            // no user accounts at all, redirect to signup
            atom.commands.dispatch(
            ...
            $('#like-status-href img').attr('src', LIKE_ICON_OUTLINE);
        }
    });
});
...
export default class KpmMusicTimeStatusView {
    constructor() {
        ...
        this.unLikeStatus.setAttribute('style', 'cursor:pointer;');
    }
    ...
    async display(msg, icon, tooltip, track) {
        ...
        let serverTrack = this.musicstoreMgr.serverTrack;
        this.trackData = track;
        ...
        await getRunningTrack().then(track => {
            ...
            if (track.state !== 'notassigned' && !needsSpotifyAccess) {
                ...
                if (serverTrack && serverTrack.loved) {
                    footerBars[0].getItem().leftPanel.appendChild(this.unLikeStatus);   
                } else {
