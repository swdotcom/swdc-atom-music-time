...
export default class StructureView {
    ...
    showLoader(isRecommendTree, isDisconnect = false) {
        if (!isRecommendTree) {
            $('#tree-content').hide();
            if (isDisconnect) {
            ...
            console.log('showLoader - showing loading');
            $('#loaderdiv').show();
        }
    }
    ...
    hideLoader() {
        $('#tree-content').show();
        console.log('hideLoader - hiding loading');
        $('#loaderdiv').hide();
    }
    getUnique(arr, comp) {
        const unique = arr
    ...
    initializeHasAccess(isRecommendTree) {
        this.showLoader(isRecommendTree);
        ...
        data.forEach((item, index) => {
            if (item) {
                ...
                if (item.id !== SPOTIFY_LIKED_SONGS_PLAYLIST_ID) {
                    ...
                    ${shareTooltip}
                '><img width='11' height='11' class='share-playlist share-parent-icon'
                node-text='${item.name}' title='share ${item.name}'
                id='share-image-${item.id}' node-id='${item.id}' src='${SHARE_ICON}' />
                </span>`;
                }
                ...
                if (item.type == 'playlist') {
                    entry = `<li node-id="${
                        item.id
                    }" class="playlist-parent-li list-nested-item expanded
                    list-item play-list ${arrowClass}  ${isCollapsedClass}" title="${
                        item.name
                ...
                ${shareTooltip}
              '><img width='11' height='11' class='share-song share-icon' node-text='${childItem.name}'
              title='share ${childItem.name}' id='share-image-${childItem.id}'
              node-id='${childItem.id}' src='${SHARE_ICON}'/>
              </span>`;
                            let addSongInnerHTML = `<span class='playlist-control' title='
              ${addTooltip}
            '><img class='add-song share-icon' node-parent-id='${item.id}'
            title='Add ${childItem.name}' id='add-image-${childItem.id}'
            node-id='${childItem.id}' src='${ADD_ICON}'/>
            </span>`;
                ...
                if (item.type == 'divider' && index > 0) {
                    entry = `<li>&nbsp;&nbsp;&nbsp;&nbsp;<span><img width='11' height='11' class='divider' src='${DIVIDER}'/>
          </span> </li>`;
                }
