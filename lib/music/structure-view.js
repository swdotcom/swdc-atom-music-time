...
export default class StructureView {
    ...
    async initialize(
        ...
        if (needsSpotifyAccess || premiumAccountRequired) {
            ...
            this.toggleRecommendationRefreshTreeview(true);
            ...
            this.toggleRecommendationRefreshTreeview(false);
        ...
        let array = [];
            if (item) {
                ...
                if (item.type == 'playlist') {
                    entry = `<li node-id="${
                        item.id
                    }" class="playlist-parent-li list-nested-item expanded  list-item play-list " style="
