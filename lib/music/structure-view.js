    ...
    treeGenerator(data) {
        ...
        _forEach(data, (item, index) => {
            ...
            '><img class='add-song share-icon' node-parent-id='${item.id}' title='Add ${childItem.name}' id='add-image-${childItem.id}' node-id='${childItem.id}' src='${ADD_ICON}'/>
                        ...
                        if (item.id == SPOTIFY_LIKED_SONGS_PLAYLIST_ID) {
                            removeSongInnerHTML = `<span class='playlist-control' title='${addTooltip}'><img class='remove-song share-icon' node-parent-id='${item.id}' title='Remove ${childItem.name}' id='remove-image-${childItem.id}' node-id='${childItem.id}' src='${REMOVE_ICON}'/></span>`;
                        } 
