...
export class MusicDataManager {
    ...
    removeTrackFromRecommendations(trackId) {
        let foundIdx = -1;
        for (let i = 0; i < this._recommendationTracks.length; i++) {
            if (this._recommendationTracks[i].id === trackId) {
                foundIdx = i;
                break;
            }
        }
        if (foundIdx > -1) {
            // splice it out
            this._recommendationTracks.splice(foundIdx, 1);
        }

        if (this._recommendationTracks.length < 2) {
            // refresh
