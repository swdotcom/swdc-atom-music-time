...
$(document).on('change', '#addPlaylist', function() {
    $(this).val() ? localStorage.setItem('selectedAddPlaylistId' , $(this).val()) : localStorage.setItem('selectedAddPlaylistId' , '');
...
export default class KpmMusicControlManager {
    ...
    async addToPlaylistMenu() {
        ...
        let selectedAddPlaylistId = localStorage.getItem('selectedAddPlaylistId');
        ...
        const musicMgr = KpmMusicManager.getInstance();
        this.currentTrackToAdd = musicMgr._playlistMap[
            _selectedAddTrackParentId
        ];
        ...
        if (selectedAddPlaylistId) {
            ...
            const matchingPlaylists = playlists.filter(n => n.id === selectedAddPlaylistId).map(n => n);
            if (matchingPlaylists.length) {
                ...
                if (matchingPlaylist) {
                    ...
                    let errMsg = null;
                    let playlistItem = {};
                    if(_selectedAddTrackParentId === SPOTIFY_RECOMMENDATION_SONGS_PLAYLIST_ID) {
        ...
        playlists = playlists
            .filter(
                n => n.itemType === 'playlist' && n.name !== 'Software Top 40' && n.name !== PERSONAL_TOP_SONGS_NAME && n.id !== _selectedAddTrackParentId
            )
            .map(n => n);
