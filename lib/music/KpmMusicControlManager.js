...
import { shareSlackSongOrPlaylist, hasSlackWorkspaces } from "../managers/SlackManager";
import {
    isResponseOk,
    softwareGet,
    softwarePut,
...
export default class KpmMusicControlManager {
    ...
    shareSong() {
        ...
        const _self = this;
        let slackStatus = hasSlackWorkspaces();
    ...
    async shareToSlack() {
        utilMgr.clearNotification();
        shareSlackSongOrPlaylist(this._selectedShareId, this.isPlaylist);
    }
    ...
    async setLiked(liked, overrideTrack = null) {
        ...
        if (liked) {
            const result = await saveToSpotifyLiked([track.id]);
        } else {
        ...
        setTimeout(() => {
          musicMgr.refreshPlaylists(false);
        }, 1000);
    }
    ...
    async connectSpotify() {
      ...
      fileUtil.setAuthCallbackState(auth_callback_state);
