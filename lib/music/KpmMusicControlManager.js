...
export default class KpmMusicControlManager {
    ...
    shareSong() {
        this._selectedSharePlaylistTrackId = localStorage.getItem(
        ...
        let shareButton = [
            {
                ...
                'text' : 'Facebook',
                'onDidClick' :  this.shareToFacebook(this._selectedSharePlaylistTrackId)
            },
            {
                'className' : 'btn btn-info',
                'text' : 'Whatsapp' 
        ...
        utilMgr.notifyButton('Music Time', 'Select Share' , shareButton)

    }
    shareToFacebook(selectedSharePlaylistTrackId) {
        const socialShare = SocialShareManager.getInstance();
        const url = buildSpotifyLink(selectedSharePlaylistTrackId, true);
