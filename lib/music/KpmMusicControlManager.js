...
export default class KpmMusicControlManager {
    ...
    async launchTrackPlayer(playerName = null, callback = null) {
        ...
        let result = await launchPlayer(playerName, { quietly: true });
        ...
        if (
            ...
            const isPrem = await KpmMusicStoreManager.getInstance().isSpotifyPremium();
    ...
    async addToPlaylistMenu() {
        ...
        if (selectedAddPlaylistId && selectedAddPlaylistId == 'playlist') {
            ...
            $('#createPlaylistText').show();
            $('#createPlaylist').show();
            
            $('#addPlaylistSelect').hide();
            return;
        }
        ...
        //     ? `${this.currentTrackToAdd.artist} - ${this.currentTrackToAdd.name}`
        //     : 'New Playlist';

        var createPlaylistButton = document.createElement('button');
        createPlaylistButton.setAttribute('style', 'display:none;');
        ...
        inputText.setAttribute(
            'style',
            ' width: 100%; display: flex;color: black;height: 30px; margin-bottom: 10px; display:none'
        );
    ...
    createNewPlaylist() {
        ...
        const placeholder = hasPlaylistItemToAdd
            ? `${this.currentTrackToAdd.artist} - ${this.currentTrackToAdd.name}`
            : 'New Playlist';
        // let playlistName = await this.showCreatePlaylistInputPrompt(
        //     placeholder
        // );
