...
function createNewPlaylist(newPlaylistName) {
    ...
    if (!newPlaylistName || newPlaylistName.trim() == '') {
        ...
        // );
        // $('#createPlaylistText').addClass('error');
        return;
    }
    // if(newPlaylistName == 'Liked Songs') {
    //     closeCategory();
    //     utilMgr.notify(
    //         'Music Time',
    //         "Please enter a playlist name to continue."
    ...
    const playlistItems = hasPlaylistItemToAdd
        ? [musicControlMgr.currentTrackToAdd]
        : [];
    musicControlMgr.createPlaylist(
        newPlaylistName,
        playlistItems
    );
    closeCategory();
...
export default class KpmMusicControlManager {
    ...
    async next(playerName = null) {
        ...
        let device = await this.musicMgr.getActiveSpotifyDevicesButton();
    ...
    async previous(playerName = null) {
        ...
        let device = await musicMgr.getActiveSpotifyDevicesButton();
    ...
    async setLiked(liked, overrideTrack = null) {
        ...
        const track = !overrideTrack ? await musicMgr.getRunningTrack() : overrideTrack;
    ...
    async launchTrackPlayer(playerName = null, callback = null) {
        utilMgr.clearNotification();
        const {
            webPlayer,
            desktop,
        ...
        let classThis = this;
        setTimeout(async () => {
            await classThis.refreshDeviceInfo();
        }, 6000);
        let result;
        ...
        if(playerName == PlayerName.SpotifyWeb) {
            ...
            result = await launchPlayer(playerName, { quietly: false });
        }
        ...
        if (
            ...
            playerName = PlayerName.SpotifyWeb;
            // await launchPlayer(playerName, { quietly: true });
            const isPrem = await this.dataMgr.isSpotifyPremium;
            if (utilMgr.isMac() && !isPrem) {
                utilMgr.notifyButton(
                    'Music Time',
    ...
    async refreshDeviceInfo() {
        const musicMgr = KpmMusicManager.getInstance();
        await musicMgr.populateSpotifyDevices();
        const structureViewObj = new StructureView();
        let devicesFound = await musicMgr.getActiveSpotifyDevicesButton();
        structureViewObj.updateDevice(devicesFound);
    }
            ...
            if (matchingPlaylists.length) {
                ...
                if (matchingPlaylist) {
                    ...
                    if (!errMsg) {
                        ...
                        if(selectedAddPlaylistId != SPOTIFY_LIKED_SONGS_PLAYLIST_ID) {
                            atom.commands.dispatch(
                                atom.views.getView(atom.workspace),
    ...
    showNewPlaylistPopup() {
        // method may be called within a callback and "this" will be undefined !!!
        const hasPlaylistItemToAdd = this.currentTrackToAdd ? true : false;
    ...
    async createPlaylist(
        ...
        const tracksToAdd = playlistTrackItems.map(
            (item) => {
                if (item.uri) {
                    return item.uri;
                }
...
export async function showDeviceSelectorMenu() {
    ...
    if (!isPrem) {
        ...
        // get the device set
        const { webPlayer, desktop } = utilMgr.getDeviceSet();
