...
export default class KpmMusicControlManager {
    ...
    shareSong() {
        isPlaylist = localStorage.getItem('isSharePlaylist');
        ...
        let shareButton = [
            ...
            },
            {
                ...
                'onDidClick' :  function () {
                    _self.copySpotifyLink(_self._selectedSharePlaylistTrackId,false)
                }
            }
        ]
        utilMgr.notifyButton('Music Time', `Share '${this._selectedShareName}' Song` , shareButton)

    }
    shareToFacebook() {
        const socialShare = SocialShareManager.getInstance();
        const url = buildSpotifyLink(this._selectedShareId, false);
    ...
    async shareToSlack() {
        await showSlackChannelMenu(this._selectedShareId);
    }
