...
export default class KpmMusicControlManager {
    ...
    async removeSong() {
        ...
        let playlistItem = musicMgr._selectedPlaylist.filter(
            track => {
                return track.id === _selectedAddPlaylistTrackId;
            }
        )[0];
...
export async function showDeviceSelectorMenu() {
    ...
    if (devices && devices.length) {
        devices.forEach((d) => {
                let status = d.is_active ? 'Active' : 'Inactive';
                items.push({
                    text: `${d.name} - ${status}`,
    ...
    if(!isPrem) {
        ...
        const foundWebComputerDevice = devices.find((d) =>
        ...
        if ( computerDevicesFound.length === 0 ||  foundWebComputerDevice) {
            items.push({
        ...
        if (!foundWebComputerDevice && isPrem) {
            items.push({
                text: "Launch Spotify web player",
                className: 'btn btn-info',
                onDidClick: () => {
