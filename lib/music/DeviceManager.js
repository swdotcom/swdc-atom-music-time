...
deviceMgr.playTrack = async () => {
    ...
    if (
        ...
        await deviceMgr.playRecommendationsOrLikedSongsByPlaylist(
            trackId,
            isLikedSong
        );
    } else {
        // play the track using the playlist id
        await deviceMgr.playPlaylistTrack(playlistId, trackId);
    }

    setTimeout(() => {
        KpmMusicManager.getInstance().gatherMusicInfoRequest();
    }, 1000);
};

deviceMgr.playPlaylistTrack = async (playlistId, trackId) => {
    const deviceId = deviceMgr.getDeviceId();
    const result = await playSpotifyPlaylist(playlistId, trackId, deviceId);
...
deviceMgr.populateSpotifyDevices = async () => {
    ...
    if (!requiresAccess && !deviceMgr.getDeviceId()) {
