 ...
 */
    ...
    async refreshPlaylistForPlayer(playerName, serverIsOnline) {
        ...
        if (playerName === PlayerName.ItunesDesktop) {
            ...
            let found = playlists.find(element => {
                return element.id == 'Liked_Songs';
            })
            // if (!found playlists.length > 1 ) {
    ...
    async launchConfirm(devices) {
        ...
        if (!isWin && !isRunning && isPrem) {
            ...
            atom.confirm({
                message: '',
                detailedMessage: `Music Time requires a running Spotify player. Choose a player to launch.`,
                buttons: {
                    'Web Player': async () => {
    ...
    async playSelectedItem(playlistItem, isExpand = true) {
        ...
        // const devices  = await getSpotifyDevices();
        // const launchConfirmInfo = await this.launchConfirm(devices);
