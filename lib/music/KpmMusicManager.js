...
export default class KpmMusicManager {
    ...
    async playPlaylistSong() {
        ...
        let isPlaylist = localStorage.getItem('isPlaylist') ? parseInt(localStorage.getItem('isPlaylist')) : 0;
        ...
        if (this._selectedPlaylistTrackId && !isPlaylist) {
            ...
            playlistItem = this._playlistMap[this._selectedPlaylistId].child.filter(element => {
 ...
 */
        ...
        if (
            ...
            filteredSpotifyPlaylists = filteredSpotifyPlaylists.map(element => {
                ...
                if(element.itemType == 'playlist') {
                    _self.getPlaylistItemTracksForPlaylistId(element.id).then(res=> {
                        element.child = res;
    ...
    async syncControls(track) {
        ...
        if (selectedPlaylist) {
            await this.clearPlaylistTracksForId(
                selectedPlaylist.id
            );
