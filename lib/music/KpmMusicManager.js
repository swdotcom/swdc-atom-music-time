 ...
 */
    ...
    async showSpotifyPlaylists(needsRefresh = false, isRecommendTree) {
        ...
        if (!requiresSpotifyAccess) {
            if (this.dataMgr.recommendationTracks && this.dataMgr.recommendationTracks.length) {
                console.log("rec label: ", this.dataMgr.recommendationLabel);
    ...
    async refreshPlaylistForPlayer(playerName) {
        ...
        if (playerName === PlayerName.ItunesDesktop) {
            ...
            if (!this.dataMgr.trackIdsForRecommendations || this.dataMgr.trackIdsForRecommendations.length === 0) {
                console.log("refreshPlaylistForPlayer - building tracks for recommendations");
    ...
    async showPlaylistItems(playlist_id) {
        ...
        if (playlist_id === SPOTIFY_LIKED_SONGS_PLAYLIST_ID) {
            ...
            console.log("rec playlist id, this.dataMgr.recommendationLabel: ", this.dataMgr.recommendationLabel);
      ...
      async populateTracksInTree(playlist_id) {
        console.log("populating tracks in tree");
        let foundPlaylist = this._spotifyPlaylists
            ? this._spotifyPlaylists.find(element => {
                  return element.type === 'playlist';
              })
    ...
    async refreshRecommendations() {
        if (utilMgr.requiresSpotifyAccess()) {
          ...
          console.log("refreshing recs, invoking update recommendations using: ", this.dataMgr.currentRecMeta.label);
    ...
    async updateRecommendations(
        ...
        console.log("updating recommendations: ", this.dataMgr.currentRecMeta);
