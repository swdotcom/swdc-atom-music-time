...
export default class KpmMusicManager {
    ...
    async gatherMusicInfo() {
        ...
        if (requiresAccess) {
            ...
            // no need to fetch tracks if we require access
            return;
        }
        ...
        const interval = playingTrack && playingTrack.id ? 6000 : 10000;
 ...
 */
    ...
    async sendMusicData(trackData) {
        ...
        console.log(`music-time: song session: ${JSON.stringify(trackData)}`);
        utilMgr.logIt(
    ...
    async showSpotifyPlaylists(needsRefresh = false, isRecommendTree) {
        ...
        if (!requiresSpotifyAccess) {
            if (!this.dataMgr.recommendationTracks || this.dataMgr.recommendationTracks.length === 0) {
                // refresh the recommended tracks
            ...
            this.structureViewObj.initializeNeedsAccess();
        }
    }
    ...
    async refreshPlaylistForPlayer(playerName) {
        if (!this.spotifyUser || !this.spotifyUser.uri) {
    ...
    async showPlaylistItems(playlist_id) {
        ...
        if (playlist_id === SPOTIFY_LIKED_SONGS_PLAYLIST_ID) {
            ...
            if (!this.hasTracks(this.dataMgr.getRecommendedTracks)) {
              // fetch the recommended tracks
              await this.refreshRecommendations();
            }

            const recommendTrackItem = this.getRecommendationPlaylistFolder();
            recommendTrackItem.childs = this.dataMgr.recommendationTracks;
    ...
    checkSpotifySongState(track_uri) {
        ...
        checkSpotifyStateTimeout = setTimeout(async () => {
            ...
            const playingTrack = this.dataMgr.runningTrack;
    ...
    async updateSpotifyAccessInfo(spotifyOauth) {
        if (spotifyOauth) {
            ...
            if (!this.spotifyUser || !this.spotifyUser.uri) {
                await this.populateSpotifyUserProfile();
            }
        } else {
    ...
    async populateSpotifyUserProfile() {
        if (!this.spotifyUser || !this.spotifyUser.uri) {
            // get the user
            this.spotifyUser = await getUserProfile();
        }
