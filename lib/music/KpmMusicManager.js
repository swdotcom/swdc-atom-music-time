...
export default class KpmMusicManager {
    ...
    hasSpotifyPlaybackAccess() {
        let hasSpotifyPlaybackAccess =
            this.spotifyUser && this.spotifyUser.product === 'premium'
                ? true
                : false;
    ...
    hasSpotifyUser() {
        let hasSpotifyUser =
            this.spotifyUser && this.spotifyUser.product ? true : false;
    ...
    isSpotifyPremium() {
        let isSpotifyPremium =
            this.hasSpotifyUser() && this.spotifyUser.product === 'premium'
                ? true
                : false;
    ...
    async gatherMusicInfo() {
        ...
        try {
            ...
            if (this.isValidTrack(playingTrack)) {
                if (playingTrack.uri) {
            ...
            if (isNewTrack) {
                ...
                if (trackStateChanged) {
                    ...
                    deviceMgr.populateSpotifyDevices();
                }
            }
 ...
 */
    ...
    async showSpotifyPlaylists(
        ...
        const needsAccess = utilMgr.requiresSpotifyAccess();
        this._selectedPlaylistId =
            this._selectedPlaylistId ||
        ...
        let foundPlaylist = this._spotifyPlaylists
            ...
            : null;
        if (!needsAccess && (!foundPlaylist || needsRefresh)) {
            await this.refreshPlaylistForPlayer(
        ...
        const hasSpotifyPlaylists =
            this._spotifyPlaylists && this._spotifyPlaylists.length
                ? true
                : false;
        const hasLikedSongs =
        ...
        // update liked songs
        if (!needsAccess && (needsRefresh || !hasSpotifyPlaylists)) {
        ...
        if (!needsAccess) {
            ...
            let needsSpotifyAccess = utilMgr.requiresSpotifyAccess();
    ...
    async getPlaylistItemTracksForPlaylistId(playlist_id) {
        ...
        if (!playlistItemTracks || playlistItemTracks.length === 0) {
            if (this._currentPlayerName === PlayerName.ItunesDesktop) {
                console.log(
                    'fetching itunes playlist tracks using playlist ID: ',
                    playlist_id
                );
        ...
        if (isTrack) {
            if (isLikedSongsPlaylist || isRecommondationSongsPlaylist) {
                ...
                playSpotifyPlaylist(
                    ...
                    if (
                        res.error.response &&
                        res.error.response.data.error.reason == 'UNKNOWN'
                    ) {
                        utilMgr.notify(
                            'Music Time',
                            'We were unable to play the selected track, because it is unavailable in your market.'
                        );
                    }
                });
            }
    ...
    async updateCodyConfig() {
        ...
        if (!this.dataMgr.spotifyClientId) {
            ...
            if (serverIsOnline) {
                ...
                if (isResponseOk(resp)) {
                    ...
                    this.dataMgr.spotifyClientId = resp.data.clientId;
                    this.dataMgr.spotifyClientSecret = resp.data.clientSecret;
                }
            }
        }
        ...
        codyConfig.spotifyClientId = this.dataMgr.spotifyClientId;
        codyConfig.spotifyAccessToken = accessToken;
        codyConfig.spotifyRefreshToken = refreshToken;
        codyConfig.spotifyClientSecret = this.dataMgr.spotifyClientSecret;
        await setConfig(codyConfig);
    ...
    async updateRecommendations(
        ...
        if (
            !this.dataMgr.recommendationTracks ||
            this.dataMgr.recommendationTracks.length === 0 ||
            this.dataMgr.rectype != this.dataMgr.currentRecMeta.label
        ) {
            if (label) {
    ...
    async getTrackIdsForRecommendations(likedSongSeedLimit = 5, offset = 0) {
        ...
        if (trackRecs.length === 0) {
            ...
            await utilMgr.buildTracksForRecommendations(this.userPlaylist);
