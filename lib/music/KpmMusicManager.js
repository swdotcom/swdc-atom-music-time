 ...
 */
    ...
    async initializeSpotify() {
        ...
        if (requiresSpotifyAccess) {
            ...
            await utilMgr.getMusicTimeUserStatus();
        } else {
            ...
            if (requiresReAuth) {
                const kpmMusicControlMgr = KpmMusicControlManager.getInstance();
                // remove their current spotify info and initiate the auth flow
                await kpmMusicControlMgr.disconnectSpotify();
                ...
                if (confirm) {
                    // now launch re-auth
                    await kpmMusicControlMgr.connectSpotify();
                }
            } else {
    ...
    async initializeSpotifyOld() {
        ...
        const spotifyOauth = {
            access_token: fileUtil.getItem('spotify_access_token'),
            refresh_token: fileUtil.getItem('spotify_refresh_token'),
        };
        await this.updateSpotifyAccessInfo(spotifyOauth);

        if (!requiresSpotifyAccess) {
            this.structureViewObj.preInitialize();
