...
export default class KpmMusicManager {
    ...
    async playPlaylistSong(isRecommondationSongs = false) {
        ...
        if(isRecommondationSongs) {
            ...
            this._selectedPlaylist = this._playlistMap[this._selectedPlaylistId].childs;
        }
        else {
            this._selectedPlaylistId = localStorage.getItem('_selectedPlaylistId');
        }
    ...
    async gatherMusicInfo() {
        ...
        try {
            ...
            if (
                ...
                utilMgr.isMac() && utilMgr.getItem('isSpotifyConnected') 
            ) {
 ...
 */
    ...
    async showSpotifyPlaylists(serverIsOnline, needsRefresh = false) {
        ...
        if (
            ...
            if(dataMgr.recommendationTracks && dataMgr.recommendationTracks.length > 0) {
            ...
            const likedTracks = await getSpotifyLikedSongs();
            ...
            let filteredSpotifyPlaylists = this._spotifyPlaylists.filter(
                element => {
                    return (
                        ...
                        element.type == 'divider'|| element.type == 'sectionDivider' || element.type == 'sectionDivider' 
                    );
                }
            );
    ...
    async refreshPlaylistForPlayer(playerName, serverIsOnline) {
        ...
        if (playerName === PlayerName.ItunesDesktop) {
            ...
            utilMgr.buildTracksForRecommendations(playlists);
    ...
    async showPlaylistItems(playlist_id, serverIsOnline) {
        ...
        let filteredSpotifyPlaylists = this._spotifyPlaylists.filter(
            element => {
                return (
                    (element.type && element.type === 'playlist') ||
                    element.type == 'divider'|| element.type == 'sectionDivider'
                );
            }
        );
    ...
    async generateUsersWeeklyTopSongs() {
        ...
        if (playlistId) {
            ...
            if (
                this.userTopSongs &&
                this.userTopSongs.length > 0
            ) {
                let tracksToAdd = this.userTopSongs.map(
                    item => {
                        if (item.uri) {
                            return item.uri;
    ...
    //             initialValue.keystrokes += parseInt(element.keystrokes);
    //             if (element.source) {
    ...
    launchAndPlaySpotifyWebPlaylistTrack = async isTrack => {
        ...
        const isRecommondationSongsPlaylist = selectedPlaylist.id === SPOTIFY_RECOMMENDATION_SONGS_PLAYLIST_ID;
    ...
    async getTrackIdsForRecommendations(
        ...
        if (trackRecs.length === 0) {
            ...
            await utilMgr.buildTracksForRecommendations(dataMgr.spotifyPlaylists);
