...
export default class KpmMusicManager {
    ...
    getChangeStatus(playingTrack, utcLocalTimes) {
        ...
        const onRepeatStartingOver = utilMgr.isOnRepeatStartingOver(playingTrack, this.trackProgressInfo);

        // get the flag to determine if the track is done or not
        const trackIsDone = utilMgr.trackIsDone(playingTrack);
        const isLongPaused = utilMgr.trackIsLongPaused(playingTrack, this.trackProgressInfo);
    ...
    async gatherMusicInfo() {
        ...
        try {
            ...
            if (isValidTrack) {
                if (!playingTrack.uri) {
                    playingTrack.uri = utilMgr.createUriFromTrackId(playingTrack.id);
                }
                playingTrack.id = utilMgr.createSpotifyIdFromUri(playingTrack.id);
            }
    ...
    async gatherCodingDataAndSendSongSession(songSession) {
        ...
        const payloads = await offlineMgr.getDataRows(utilMgr.getMusicSessionFile());
 ...
 */
    ...
    async playSelectedItem(playlistItem, isExpand = true) {
        ...
        if (playlistItem && playlistItem.type === 'track') {
            ...
            this._selectedPlaylist = playlistItem;
