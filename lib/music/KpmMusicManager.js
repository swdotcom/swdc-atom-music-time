...
export default class KpmMusicManager {
    ...
    async togglePLaylist() {
        ...
        if (isCollapsed) {
            ...
            this._spotifyPlaylists.map( playlist => {
    ...
    hasSpotifyPlaybackAccess() {
        let hasSpotifyPlaybackAccess =  this.spotifyUser && this.spotifyUser.product === 'premium'
            ? true
            : false;
    ...
    hasSpotifyUser() {
        let hasSpotifyUser =  this.spotifyUser && this.spotifyUser.product ? true : false;
        this.dataMgr.hasSpotifyUser = hasSpotifyUser;
        return hasSpotifyUser;
    }

    isSpotifyPremium() {
        let isSpotifyPremium = this.hasSpotifyUser() && this.spotifyUser.product === 'premium'
            ? true
            : false;
    ...
    async gatherMusicInfo() {
        ...
        try {
            ...
            this.structureViewObj.updateDevice(devicesFound);
 ...
 */
    ...
    async showSpotifyPlaylists(serverIsOnline, needsRefresh = false, isRecommendTree) {
        ...
        if (
            ...
            if(this._selectedPlaylistId === SPOTIFY_LIKED_SONGS_PLAYLIST_ID ) {
                this._selectedPlaylist = await getSpotifyLikedSongs();
            } else {
                this._selectedPlaylist = await this.getPlaylistItemTracksForPlaylistId(
                    this._selectedPlaylistId
                );
            ...
            if (
                ...
                this.dataMgr.recommendationTracks.length > 0
            ) {
    ...
    async getActiveSpotifyDevicesButton() {
        ...
        let webPlayerDevice = devices.find(
            d =>
                d.type.toLowerCase() === 'computer' &&
                d.name.toLowerCase().includes('web player')
        );
        if(utilMgr.isMac() && !this.dataMgr.isSpotifyPremium) {
            webPlayerDevice = null;
        }
        ...
        if (activeDevice) {
            if(activeDevice.type.toLowerCase() === 'computer' && activeDevice.name.toLowerCase().includes('web player') && !this.dataMgr.isSpotifyPremium) {
    ...
    async updateSort(sortAlpha) {
        // utilMgr.notify('Music Time', 'Sorting playlists, please wait.');
    ...
    async refreshPlaylistForPlayer(playerName, serverIsOnline) {
        ...
        if (this.dataMgr.savedPlaylists && this.dataMgr.savedPlaylists.length === 0) {
            // fetch and reconcile the saved playlists against the spotify list
            await this.dataMgr.fetchSavedPlaylists(serverIsOnline);
        }
        ...
        if (playerName === PlayerName.ItunesDesktop) {
            ...
            // this.dataMgr.getAiTopFortyPlaylist();
    ...
    async showPlaylistItems(playlist_id, serverIsOnline) {
        ...
        if (playlist_id === SPOTIFY_LIKED_SONGS_PLAYLIST_ID) {
            ...
            if (
                ...
                this._selectedPlaylist = this.dataMgr.recommendationTracks;
            }
        } else {
        ...
        filteredSpotifyPlaylists = filteredSpotifyPlaylists.map(element => {
            if(this._selectedPlaylistId == element.id) {
    ...
    getPlaylistItemTracksFromCodyResponse(codyResponse) {
        ...
        return playlistItems.length > 0 ? playlistItems : this.createPlaylistItemForEmptyPlaylist();;
    }
    ...
    retrieveMusicTimePlaylist(playlists) {
        this._musictimePlaylists = [];
        if (
            (this.dataMgr.savedPlaylists && this.dataMgr.savedPlaylists.length > 0) ||
            playlists.length > 0
        ) {
    ...
    async isComputerDeviceRunning(devices) {
        ...
        if(activeDevice && activeDevice.type.toLowerCase() === 'computer' && activeDevice.name.toLowerCase().includes('web player') && !this.dataMgr.isSpotifyPremium) {
            isRunning = false;
        }
        return isRunning;
    }
    ...
    async launchConfirm(devices, playlistItem) {
        ...
        if (
            ...
            utilMgr.notify(
                'Music Time',
                "Desktop player is not installed. Please install it."
            );
            if(!isPrem) {
                proceed = false;
                isLaunching = false;
    ...
    launchAndPlaySpotifyWebPlaylistTrack = async isTrack => {
        ...
        //     // $('#music-prev-control-status span img').prop("src", false);
        //     $('#music-prev-control-status').prop("onclick", false);
        // }
        ...
        if (isTrack) {
            if (isLikedSongsPlaylist || isRecommondationSongsPlaylist) {
                ...
                playSpotifyPlaylist(
                    ...
                    if(res.error.response && res.error.response.data.error.reason == 'UNKNOWN') {
                        utilMgr.notify('Music Time','We were unable to play the selected track, because it is unavailable in your market.');
                    }
                });
            }
        }
    ...
    async launchAndPlayTrack(track, spotifyUser) {
        const musicCtrlMgr = KpmMusicControlManager.getInstance();
        const currentPlaylist = this._selectedPlaylist;
        // check if there's any spotify devices
        const spotifyDevices = this.dataMgr.currentDevices;
    ...
    async refreshRecommendations() {
        ...
        } else if (this.dataMgr.currentRecMeta && this.dataMgr.currentRecMeta.label) {
    ...
    async updateRecommendations(
        ...
        let isChangeMoodOrGenre = false;
        if(!this.dataMgr.recommendationTracks || this.dataMgr.recommendationTracks.length === 0 || this.dataMgr.rectype != this.dataMgr.currentRecMeta.label) {
            if (label) {
                this.dataMgr.rectype = label;
            }
            const trackIds = await this.getTrackIdsForRecommendations(
                likedSongSeedLimit,
                offset
            );
            this.dataMgr.allRecommendationTracks = await this.getRecommendedTracks(
                trackIds,
                this.dataMgr.currentRecMeta.seed_genres,
                this.dataMgr.currentRecMeta.features
            );
            isChangeMoodOrGenre = true;
        }

        // if (this.dataMgr.allRecommendationTracks.length > 0) {
        //     // sort them alpabeticaly
        //     this.dataMgr.allRecommendationTracks = utilMgr.sortTracks(this.dataMgr.allRecommendationTracks);
        // }

        // get the tracks that have already been recommended
        let existingTrackIds = this.dataMgr.prevRecTrackMap[
            this.dataMgr.currentRecMeta.label
        ]
            ? this.dataMgr.prevRecTrackMap[this.dataMgr.currentRecMeta.label]
            : [];
        let finalTracks = [];
        if (existingTrackIds.length && isRefresh) {
            // filter out the ones that are already used
            this.dataMgr.allRecommendationTracks.forEach(track => {
            ...
            if (finalTracks.length < 10) {
                ...
                finalTracks = [];
                finalTracks.push(...this.dataMgr.allRecommendationTracks);
                // clear out the old
                existingTrackIds = [];
            }
        } else {
        ...
        // this.dataMgr.allRecommendationTracks = finalTracks;
        // trim down to 10
        ...
        this.dataMgr.recommendationTracks = splicedTracks;
        this.dataMgr.recommendationLabel = this.dataMgr.currentRecMeta.label;
        if(isChangeMoodOrGenre) {
    ...
    playRecommendationsOrLikedSongsByPlaylist = (
        ...
        if (isRecommendationTrack) {
            ...
            if (this.dataMgr.playingTracks && this.dataMgr.playingTracks.length > 0) {
            ...
            let allRecommendationTrackId = this.dataMgr.allRecommendationTracks.map( track => track.id );
