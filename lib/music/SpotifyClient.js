    ...
    if (confirm) {
        ...
        if (isResponseOk(result)) {
            ...
            if (type_lc === 'spotify') {
                ...
                userStatusManager.updateSpotifyAccessInfo(null);
...
spotifyClient.spotifyConnectStatusHandler = async (tryCountUntilFound) => {
    const oauthResult = await userStatusManager.getUserRegistrationState();
...
spotifyClient.initializeSpotify = async () => {
    ...
    if (requiresSpotifyAccess) {
        ...
        await userStatusManager.getUserRegistrationState();
    }
