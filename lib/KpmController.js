...
export class KpmController {
    ...
    constructor() {
        ...
        if (!this.rootPath) {
            if (!this.keystrokeMgr) {
                ...
                this.keystrokeMgr = new KeystrokeManager(this.defaultName, this.defaultName);
            }
        ...
        const lastSlashIdx = this.rootPath ? this.rootPath.lastIndexOf('/') : -1;
        const projectName =
            lastSlashIdx !== -1
                ? this.rootPath.substring(this.rootPath.lastIndexOf('/') + 1)
                : this.rootPath;

        if (this.rootPath && this.keystrokeMgr && !this.keystrokeMgr.hasDirectory()) {
            ...
            this.keystrokeMgr = new KeystrokeManager(projectName, this.rootPath);
        }
    ...
    async sendKeystrokeDataIntervalHandler() {
        ...
        await this.initializeKeystrokesCount(staticInfo.filename, this.rootPath);
    ...
    async processOfflineKeystrokes(sendCurrentKeystrokes = false) {
        const isonline = await utilMgr.serverIsAvailable();
        if (!isonline) {
            return;
        }
        let payloads = [];
                ...
                if (content) {
                    ...
                    for (let i = 0; i < payloads.length; i++) {
                        if (batch.length >= batch_limit) {
                            await utilMgr.sendBatchPayload(batch);
                            batch = [];
                        }
