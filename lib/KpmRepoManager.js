        ...
        if (devList && devList.length > 0) {
            ...
            softwarePost(
                ...
                if (isResponseOk(resp)) {
                    ...
                    console.log('Music Time: repo membership updated');
                }
            });
        }
    }
...
function sendCommits(commitData) {
    // send this to the backend
    softwarePost('/commits', commitData, fileUtil.getItem('jwt')).then(resp => {
        if (isResponseOk(resp)) {
            if (resp.data) {
                console.log(`Music Time: ${resp.data.message}`);
            } else {
                // everything is fine, delete the offline data file
                console.log('Music Time: repo commits updated');
            }
        }
    });
}
