...
utilMgr.showErrorStatus = async () => {
    utilMgr.getStatusView().display('Music Time');
};
...
utilMgr.showDeactivatedErrorStatus = async () => {
    utilMgr
        ...
        .display(
            'Music Time',
            'alert',
            'To see your coding data in Music Time, please reactivate your account.'
        );
};
...
utilMgr.isDeactivated = async () => {
    let pingResp = await softwareGet('/users/ping/', utilMgr.getItem('jwt'));
...
utilMgr.sendSessionPayload = songSession => {
    ...
    return softwarePost(api, songSession, utilMgr.getItem('jwt'))
        .then(resp => {
            if (!isResponseOk(resp)) {
                return { status: 'fail' };
            }
...
utilMgr.sendPreferencesUpdate = async (userId, userPrefs) => {
    ...
    let resp = await softwarePut(api, userPrefs, utilMgr.getItem('jwt'));
    if (isResponseOk(resp)) {
...
utilMgr.getUserId = async () => {
    ...
    const email = utilMgr.getItem('name');
    if (jwt && email) {
        let api = `/users/me`;
        let resp = await softwareGet(api, jwt);
        if (isResponseOk(resp)) {
...
utilMgr.sendHeartbeat = async reason => {
    ...
    if (jwt) {
        ...
        softwarePost(api, heartbeat, jwt).then(async resp => {
            if (!isResponseOk(resp)) {
                console.log('Music Time: unable to send heartbeat ping');
            }
        });
    }
};
...
utilMgr.spotifyConnectStatusHandler = tryCountUntilFound => {
    ...
    return __awaiter(this, void 0, void 0, function*() {
        const oauthResult = yield utilMgr.getMusicTimeUserStatus();
...
utilMgr.getMusicTimeUserStatus = async () => {
...
utilMgr.buildTracksForRecommendations = async playlists => {
    ...
    if (dataMgr.spotifyLikedSongs && dataMgr.spotifyLikedSongs.length > 1) {
