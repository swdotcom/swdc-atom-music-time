...
utilMgr.sendSessionPayload = songSession => {
    ...
    return softwarePost(api, songSession, fileUtil.getItem('jwt'))
        .then(resp => {
            if (!isResponseOk(resp)) {
                return { status: 'fail' };
            }
...
utilMgr.sendBatchedLikedSongSessions = tracksToSave => {
    ...
    softwarePut(api, { tracks: tracksToSave }, fileUtil.getItem('jwt'))
        .then(resp => {
            if (!isResponseOk(resp)) {
                return { status: 'fail' };
            }
...
utilMgr.displayReadmeIfNotExists = (override = false) => {
    const displayedReadme = fileUtil.getItem('displayedMtReadme');
