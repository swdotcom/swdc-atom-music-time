...
utilMgr.sendSessionPayload = (songSession) => {
    ...
    return utilMgr.softwarePost(api, songSession, utilMgr.getItem("jwt"))
        .then(resp => {
            if (!utilMgr.isResponseOk(resp)) {
                return { status: "fail" };
            }
...
utilMgr.notify = (title, msg, dismissable = true) => {
    utilMgr.clearNotification();
    atom.notifications.addInfo(title, { detail: msg, dismissable: dismissable });
};
