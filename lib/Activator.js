...
export default {
    ...
    async initializePlugin(state, initializedUser) {
        ...
        // gather device info
        this.deviceCheckInterval = settInterval(() => {
    ...
    async initializeSpotifyUser(tries = 0) {
        ...
        const spotifyUser = await musicMgr.populateSpotifyUserProfile();
        if (tries > 2 || (spotifyUser && spotifyUser.uri)) {
            // we have a user, initialize the playlist
            setTimeout(() => {
              this.initializeSpotifyPlaylist();
            }, 2000);

        } else {
            tries++;
            setTimeout(() => {
                this.initializeSpotifyUser(tries);
            }, 3000);
        }
    },
    ...
    async initializeSpotifyPlaylist() {
        ...
        const initSpotifyWait = deviceMgr.getDeviceId() ? 2000 : 5000;
        setTimeout(async () => {
