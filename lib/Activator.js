...
export default {
    ...
    async initializePlugin(state, initializedUser) {
        ...
        this.subscriptions.add(
            atom.commands.add('atom-workspace', {
                'Music-Time:createNewPlaylist': () => {
                    let newPlaylistName = localStorage.getItem(
                        'newPlaylistName'
                    );
        ...
        this.subscriptions.add(
            atom.commands.add('atom-workspace', {
                'Music-Time:toggle-music-tree': () => this.togglePlaylistView(),
            })
        );
    ...
    togglePlaylistView() {
        ...
        this.playlistVisible = !this.playlistVisible ? true : false;
        ...
        if (this.playlistVisible) {
            try {
                ...
                dock.getPanes()[0].addItem(this.structureView);
                dock.getPanes()[0].activateItem(this.structureView);
            } catch (e) {
                //
            }
            dock.show();
        } else {
            dock.getPanes()[0].removeItem(this.structureView);
        }
    },
};
