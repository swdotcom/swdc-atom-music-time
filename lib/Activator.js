...
import { GITHUB_ISSUE_URL, FEEDBACK_URL,CONNECT_SPOTIFY_MENU_LABEL } from './Constants';
...
export default {
        ...
        this.subscriptions.add(
            atom.commands.add('atom-workspace', {
                'Music-Time:createNewPlaylist': () =>
                    musicControlMgr.createNewPlaylist(),
            })
        );
        ...
        this.subscriptions.add(
            atom.commands.add('atom-workspace', {
                'Music-Time:play-recommend-song': () =>
                    musicMgr.playPlaylistSong(true),
            })
        );
        ...
        this.subscriptions.add(
            atom.commands.add('atom-workspace', {
                'Music-Time:launchSpotifyDesktop': () => {
                    musicControlMgr.launchTrackPlayer(PlayerName.SpotifyDesktop);
                },
            })
        );
        this.subscriptions.add(
    ...
    togglePlaylistView(stat) {
        ...
        if (!this.structureView) this.structureView = new StructureView();
         if ('on' === stat) {
            // this.initializePlugin(null, false);
